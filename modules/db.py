...
def visit_progress(total_visits):
    """Return streak progress toward reward threshold."""
    threshold = config.REWARD_VISITS
    streak = total_visits % threshold
    if streak == 0 and total_visits > 0:
        streak = threshold
...
def all_customers_with_visits(conn):
    return conn.execute("""
        SELECT c.id, c.name, c.email, c.identifier, COUNT(v.id) as visits
        FROM customers c
